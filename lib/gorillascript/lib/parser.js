  ...
  _FunctionDeclaration = (function () {
    ...
    return allowSpaceBeforeAccess(function (parser, index) {
      var _arr, _i, asType, body, flags, flagsValue, func, generic,
          genericMacro, param, params, paramsRule, promiseMacro, result, scope;
      ...
      if (flagsValue.promise) {
        promiseMacro = parser.getMacroByLabel("__promise");
        if (!promiseMacro) {
          throw ParserError(
            "Cannot use promises until the promise! macro has been defined",
            parser,
            index
          );
        }
        result = promiseMacro.func({ macroData: [result] }, parser, index);
      }
