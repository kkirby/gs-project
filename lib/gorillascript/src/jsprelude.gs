...
macro for
  syntax reducer as (\every | \some | \first | \filter)?, value as Declarable, index as (",", value as Identifier, length as (",", this as Identifier)?)?, "in", array as Logic, body as (BodyNoEnd | (";", this as Statement)), else-body as ("\n", "else", this as (BodyNoEnd | (";", this as Statement)))?, "end"
    ...
    if @has-func(body)
      let func = @tmp \f, false
      if @in-generator and not (value and value-ident != value.ident)
        if has-index
          init.push AST(body) let $func = promise! #($value-ident, $index)* -> $body
          body := ASTE(body) yield $func@(this, $value-expr, $index)
        else
          init.push AST(body) let $func = promise! #($value-ident)* -> $body
          body := ASTE(body) yield $func@(this, $value-expr)
      else
        if value and value-ident != value.ident
          body := AST(body)
            let $value = $value-ident
