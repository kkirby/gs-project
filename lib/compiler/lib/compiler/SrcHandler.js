...
module.exports = (function(){
	...
	var compile = function(state,file){
		...
		if(state.compiler.config.checkForUnusedImports){
			parsedFileContents.importNames.forEach(function(importName){
				var matches = parsedFileContents.body.match(importName);
				if(matches == null){
		...
		if(!state.hasLoadedFile(file)){
			...
			if(state.watching){
				if(cache.ids[file] != cacheId){
					try {
						src = (function(defines){
							var res = [];
							for(var key in defines){
								var value = defines[key];
								res.push('const ' + key + ' = ' + JSON.stringify(value));
							}
							return res.join('\n');
						})(state.compiler.config.defines) + '\n' + src;
						src = gs.compile(
						...
						if(state.compiler.config.useRegenerator){
	...
	SrcHandler.prototype.afterProcessed = function(state){
		...
		if (state.apiProxyUrl) {
			var corsProxyOverride = '(function(open) {\n' +
				'\tif(window.proxiedXmlHttpRequest) return;\n' +
				'\tXMLHttpRequest.prototype.open = function(method, url, async, user, pass) {\n' +
				'\t\topen.call(this, method, "' + state.apiProxyUrl + '" + url, async, user, pass);\n' +
				'\t};\n' +
				'\twindow.proxiedXmlHttpRequest = true;\n' +
				'})(XMLHttpRequest.prototype.open);';
			outputSource = corsProxyOverride + '\n' + outputSource;
		}
