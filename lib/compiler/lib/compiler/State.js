...
module.exports = (function(){
	var State = function(compiler,callback,watching){
		...
		this.custom = {};
		this.html = this.compiler.config.html;
		this.hasDom = !!this.html;
	...
	State.prototype = {
		...
		getPathFromAliasAndType: function(alias,type,relativeTo){
			...
			if(alias[alias.length - 1].substr(0,1) == '*'){
				...
				for(var eI = 0; eI < extensions.length; eI++){
					for(var pI = 0; pI < path.length; pI++){
						...
						for(var mI = 0; mI < matches.length; mI++){
							ret.push(matches[mI].replace(/\//g,mPath.sep));
						}
					}
				}
				return ret;
				...
				if(!mFs.existsSync(ret)){
					throw 'Unable to find alias ' + alias + ' looked in: ' + locations.join("\n")
				}
				return ret;
			}
		},
			...
			if(typeof this.fileInfoCache[file] == 'undefined'){
				...
				for(var rootKey in this.compiler.config.alias){
					...
					if(
						...
						rootDir.length > (rootInfo.key ? rootInfo.dir.length : 0)
					){
						rootInfo.dir = rootDir;
						rootInfo.key = rootKey;
					}
				}
				info.root = rootInfo.key;
				file = file.substr(rootInfo.dir.length+1);
				var type = {
					dir: null,
					key: null
				};
				...
				info.relative = originalFile.substr(rootInfo.dir.length+1);
