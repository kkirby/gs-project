		...
		compile: function(watch,cb){
			...
			var state = new CompilerState(this,function(){
				try {
					...
					var outputFileCompiled = mPath.join(this.config.output,this.config.sourceFileName);
					...
					var contents = gs.compile(
						'GLOBAL.App := {}\n'+state.contents
					).code;
					contents = regenerator.compile(contents,{includeRuntime:true}).code;
					if(!this.config.noHelper){
					...
					if(state.hasDom){
						...
						$('body')[0].appendChild(
							$('<script/>').prop('src','source.js').prop('type','text/javascript')[0]
						);
						var html = state.domWindow.document.documentElement.outerHTML;
						if(html){
							state.saveFile(
